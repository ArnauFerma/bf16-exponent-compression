#!/usr/bin/env python3
"""Correccion + medida de las 4 variantes del kernel optimizado."""
import sys, time
...
hdr = f"{'BLOCK':>6}{'thr':>5} {'variante':>22} {'shared':>8} {'ms':>9} {'GB/s':>7} {'vs base':>8}  ok"
...
for block in (64, 128, 256, 512, 1024):
    for threads in (32, 64, 128, 256):
        ...
        # baseline: el kernel actual de gpu_kernels.py
        ...
        print(f"{block:6d}{threads:5d} {'base (global)':>22} {'-':>8} "
              f"{base_ms:8.2f} {moved/(base_ms*1e-3)/1e9:7.2f} {'1.00':>8}  {'si' if ok else 'NO'}")
